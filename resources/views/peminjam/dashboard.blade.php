                    ...
                    @forelse($pinjamanAktif as $pinjam)
                        <tr class="hover:bg-slate-50/50 transition-colors">
                            ...
                            <td class="px-6 py-4 text-center">
                                @php $diff = round(\Carbon\Carbon::now()->diffInDays($pinjam->tgl_kembali_rencana, false)); @endphp
                                @if ($diff < 0)
                                    <span
                                        class="px-3 py-1 bg-red-100 text-red-600 rounded-lg text-[10px] font-black uppercase">
                                        Terlambat {{ abs((int) $diff) }} Hari
                                    </span>
                                @elseif ($diff == 0)
                                    <span
                                        class="px-3 py-1 bg-orange-100 text-orange-600 rounded-lg text-[10px] font-black uppercase">
                                        Deadline Hari Ini!
                                    </span>
                                @else
                                    <span
                                        class="px-3 py-1 bg-blue-50 text-blue-600 rounded-lg text-[10px] font-black uppercase">
                                        {{ (int) $diff }} Hari Lagi
                                    </span>
                                @endif
                            </td>
            ...
            @foreach ($rekomendasiBuku as $buku)
                <div
                    ...
                    <div class="p-5">
                        ...
                        <h3 class="font-bold text-slate-800 truncate mt-2 group-hover:text-primary transition-colors">
                            {{ $buku->judul_buku }}</h3>
